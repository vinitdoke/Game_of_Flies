...
from acceleration_updater import (
    accelerator,
    bin_particles,
    cumsum,
    set_indices,
...
def integrate(
    ...
    step1[blocks, threads](
        vel_x, vel_y, vel_z, acc_x, acc_y, acc_z, num_particles, timestep
    )

    accelerator[blocks, threads](
        pos_x,
        pos_y,
        pos_z,
        vel_x,
    ...
    boundary_condition[blocks, threads](
        pos_x, pos_y, pos_z, limits[0], limits[1], limits[2], num_particles
    )
...
def setup_bins(
    ...
    cumsum[blocks, threads](
        particle_bin_counts, particle_bin_starts, int(np.log2(num_bins))
    )

    set_indices[blocks, threads](
        particle_bins, particle_bin_starts, bin_offsets, particle_indices, num_particles
    )
