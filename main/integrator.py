...
def integrate(
    ...
    accelerator[blocks, threads](pos_x, pos_y, pos_z, vel_x, vel_y, vel_z, limits[0], limits[1], limits[2], r_max, num_particles, parameter_matrix, particle_type_index_array,
                                acc_x, acc_y, acc_z, bin_neighbours, particle_bins, particle_indices, bin_starts, bin_counts)
    
    step2[blocks, threads](pos_x, pos_y, pos_z, vel_x, vel_y, vel_z, acc_x, acc_y, acc_z, num_particles, timestep)

    boundary_condition[blocks, threads](pos_x, pos_y, pos_z, limits[0], limits[1],limits[2], num_particles)


def setup_bins(
        pos_x, pos_y, pos_z, num_bin_x, num_bin_y, num_bin_z, bin_size_x, bin_size_y, bin_size_z, num_bins, num_particles,
        particle_bins, particle_bin_counts, bin_offsets, particle_bin_starts, particle_indices,
        blocks, threads
):
    bin_particles[blocks, threads](pos_x, pos_y, pos_z, num_bin_x, num_bin_y, num_bin_z, bin_size_x, bin_size_y, bin_size_z, num_particles,
...
if __name__ == "__main__":
    ...
    num_bin_z = int(np.floor(limits[2] / r_max))
    bin_size_x = limits[0] / num_bin_x
    bin_size_y = limits[1] / num_bin_y
    bin_size_z = limits[2] / num_bin_z

    bin_neighbours = np.zeros((num_bin_x * num_bin_y * num_bin_z, 18), dtype=np.int32)
    set_bin_neighbours(num_bin_x, num_bin_y, num_bin_z, bin_neighbours)
    

    i = 1
    while i < num_bin_x * num_bin_y * num_bin_z:
        i *= 2
    ...
    for i in range(reps + 1):
        ...
        setup_bins(d_pos_x, d_pos_y, num_bin_x, num_bin_y, num_bin_z, bin_size_x, bin_size_y, num_bins, num_particles,
